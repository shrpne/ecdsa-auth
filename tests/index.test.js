...
test('signRequest', () => {
    const signature = ecdsaAuthSign(VALID_REQUEST.data, PRIVATE_KEY);
    expect(signature).toEqual(VALID_REQUEST.signature.result);
});

test('signRequest without publicKey', () => {
    const signature = ecdsaAuthSign(VALID_REQUEST.data, PRIVATE_KEY, false);
...
test('recovery', () => {
    const {signature, recid} = ecdsaAuthParse(VALID_REQUEST.signature.result);
...
test('verify', () => {
    const {signature, publicKey} = ecdsaAuthParse(VALID_REQUEST.signature.result);
    const dataHash = hashObject(VALID_REQUEST.data);
...
describe('test case from api', () => {
    describe('1', () => {
        ...
        test('ecdsaAuthVerify', () => {
            const dataString = Buffer.from(dataHexString, 'hex').toString('utf8');
            const data = JSON.parse(dataString);

            const address = ecdsaAuthVerify(data, headerAuthResult)
            expect(address).toEqual('0x' + validAddress);
        })
        test('low level', () => {
            const dataString = Buffer.from(dataHexString, 'hex').toString('utf8');
            const data = JSON.parse(dataString);
            ...
            const {signature, recid, publicKey: publicKeyValid} = ecdsaAuthParse(headerAuthResult);
...
describe('timestamp', () => {
    ...
    test('empty get', () => {
        ...
        const auth = ecdsaAuthSign(TIMESTAMP, PRIVATE_KEY);
        ...
        const {signature, publicKey} = ecdsaAuthParse(auth);
    ...
    test('empty post', () => {
        ...
        const auth = ecdsaAuthSign(data, PRIVATE_KEY);
